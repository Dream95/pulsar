...
`WindowFunctionExecutor` passes the full `Window` object to `WindowFunction.process(window, context)`. For existing implementations, the default method delegates to `process(window.get(), context)`.

If a class-based implementation overrides both methods, the `Window<Record<X>>` overload is the runtime entry point because the executor calls `process(window, context)`. The collection-based method remains available for shared logic or fallback behavior; for example, the window overload may explicitly call `process(window.get(), context)` after using `getNew()` / `getExpired()`.
...
public interface Window<T> {
     ...
     * <p>
     * For event-time windows, this is based on the watermark. For processing-time
     * windows, this is based on processing time. Built-in trigger policies provide
     * this reference timestamp for activated windows. If no reference timestamp is
     ...
     * time-based window length. Returns {@code null} if the window length is not
     * time-based or if no end timestamp is available.
     * </p>
     */
...
public class SlidingWindowSumFunction implements WindowFunction<Integer, Integer> {
    ...
    public Integer process(Collection<Record<Integer>> input, WindowContext context) {
        // Note: When both overloads are overridden, the runtime invokes
        // process(Window, WindowContext). This method remains supported for
        // shared logic or explicit delegation (e.g. process(window.get(), context)).
...
Update `process(Window<Record<X>>, WindowContext)` to pass the full window view to `WindowFunction`. Existing class implementations and lambdas use the default overload, which delegates to `process(inputWindow.get(), context)`. Class implementations that override the new overload receive access to `getNew()` / `getExpired()`. If both overloads are implemented, the new `Window<Record<X>>` overload is called by the runtime.
...
| `Long getStartTimestamp()` | Window start time, computed from `getEndTimestamp()` and the configured time-based window length. Returns `null` if the window length is not time-based or if no end timestamp is available. |
| `Long getEndTimestamp()` | Window end time for this activation: watermark-based for event-time windows, processing-time based for processing-time windows. Built-in trigger policies provide this timestamp; returns `null` only if no reference timestamp is available. |

#### API contracts (`Window` lifetime and list mutability)

On each window trigger, `WindowFunctionExecutor` builds a new `WindowImpl` with freshly collected lists and passes it to `WindowFunction.process(Window<Record<X>>, WindowContext)`. The runtime does not retain that object after `process()` returns.
...
| `default T process(Window<Record<X>> window, WindowContext context)` | New default method. Delegates to `process(window.get(), context)` unless a class-based implementation overrides it to access incremental and expired events. |
